...
def self_play_iteration(
    ...
    """Run one self-play + training iteration."""
...
def match_win_rate(agent_one, agent_two, num_matches, alternate_starts=True):
    """Run matches and tally wins, losses, and draws from agent_one's perspective."""
    wins = 0
    losses = 0
    draws = 0
    ...
    for match in range(num_matches):
        if alternate_starts:
            starting_player = 1 if match % 2 == 0 else 2
        else:
            starting_player = 1
...
def evaluate_against_random(net, num_matches, seed=None):
    """Evaluate the greedy network agent against a reproducible random baseline."""
    rng = np.random.default_rng(seed)
    ...
    def random_agent(state, to_play):
        return random_policy_action(state, to_play, rng=rng)
